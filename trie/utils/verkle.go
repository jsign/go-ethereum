...
func GetTreeKeyStorageSlot(address []byte, storageKey *big.Int) ([]byte, error) {
	pos := new(big.Int).Set(storageKey)
	if storageKey.Cmp(codeStorageDelta.ToBig()) < 0 {
		pos.Add(HeaderStorageOffset.ToBig(), storageKey)
	} else {
		pos.Add(MainStorageMinusStorageDeltaOffset.ToBig(), storageKey)
	}
	treeIndexBigInt := new(big.Int).Div(pos, VerkleNodeWidth.ToBig())
	treeIndex, overflowed := uint256.FromBig(treeIndexBigInt)
	if overflowed {
		return nil, fmt.Errorf("treeIndex overflowed")
	}

	// calculate the sub_index, i.e. the index in the stem tree.
	// Because the modulus is 256, it's the last byte of treeIndex
	subIndexModBigInt := new(big.Int).Mod(pos, VerkleNodeWidth.ToBig())
	subIndexMod, overflowed := uint256.FromBig(subIndexModBigInt)
	if overflowed {
		return nil, fmt.Errorf("subIndex overflowed")
	}
	...
	return GetTreeKey(address, treeIndex, subIndex), nil
}
