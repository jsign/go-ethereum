...
func convertExecutionWitness(witness *stateless.Witness) (*executionWitness, error) {
	...
	for i, header := range ext.Headers {
		...
		result.Headers[len(ext.Headers)-1-i] = encoded
	}
	return result, nil
}
...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig, txIt txIterator, miningReward int64) (*state.StateDB, *ExecutionResult, []byte, error) {
	...
	var (
		...
		isAmsterdam  = chainConfig.IsAmsterdam(vmContext.BlockNumber, vmContext.Time)
	)
	if isAmsterdam {
		var err error
	...
	if isAmsterdam {
		encodedBal := blockAccessList.ToEncodingObj()
		balBytes, err := rlp.EncodeToBytes(encodedBal)
		if err != nil {
			return nil, nil, nil, NewError(ErrorEVM, fmt.Errorf("could not encode block access list: %v", err))
		}
		balHash := encodedBal.Hash()
		execRs.BlockAccessList = balBytes
